...
void set_ssm_params_fwd(SSMParamsBase &params,
                        ...
                        const c10::optional<at::Tensor> &P_,
                        const c10::optional<at::Tensor> &Q_,
                        const c10::optional<at::Tensor> &gamma_,
                        void* D_ptr,
                        void* delta_bias_ptr,
                        void* x_ptr,
                        bool has_z,
    ...
    params.has_pq = P_.has_value() && Q_.has_value();
    params.P_ptr = params.has_pq ? P_.value().data_ptr() : nullptr;
    params.Q_ptr = params.has_pq ? Q_.value().data_ptr() : nullptr;
    params.gamma_ptr = gamma_.has_value() ? gamma_.value().data_ptr() : nullptr;
    ...
    if (params.has_pq) {
        const auto P = P_.value();
        const auto Q = Q_.value();
        params.pq_rank = P.size(-1);
        params.P_d_stride = P.dim() == 3 ? P.stride(0) : 0;
        params.P_dstate_stride = P.dim() == 3 ? P.stride(1) : P.stride(0);
        params.P_r_stride = P.dim() == 3 ? P.stride(2) : P.stride(1);
        params.Q_d_stride = Q.dim() == 3 ? Q.stride(0) : 0;
        params.Q_r_stride = Q.dim() == 3 ? Q.stride(1) : Q.stride(0);
        params.Q_dstate_stride = Q.dim() == 3 ? Q.stride(2) : Q.stride(1);
    }
    if (gamma_.has_value()) {
        const auto gamma = gamma_.value();
        params.gamma_d_stride = gamma.dim() == 2 ? gamma.stride(0) : 0;
        params.gamma_dstate_stride = gamma.dim() == 2 ? gamma.stride(1) : gamma.stride(0);
    }
}
...
void set_ssm_params_bwd(SSMParamsBwd &params,
                        ...
                        const at::Tensor dC,
                        const at::Tensor dz,
                        const c10::optional<at::Tensor> &dP_,
                        const c10::optional<at::Tensor> &dQ_,
                        void* dD_ptr,
    ...
    params.dP_ptr = dP_.has_value() ? dP_.value().data_ptr() : nullptr;
    params.dQ_ptr = dQ_.has_value() ? dQ_.value().data_ptr() : nullptr;
    // All stride are in elements, not bytes.
    ...
    if (dP_.has_value()) {
        const auto dP = dP_.value();
        params.dP_d_stride = dP.dim() == 3 ? dP.stride(0) : 0;
        params.dP_dstate_stride = dP.dim() == 3 ? dP.stride(1) : dP.stride(0);
        params.dP_r_stride = dP.dim() == 3 ? dP.stride(2) : dP.stride(1);
    }
    if (dQ_.has_value()) {
        const auto dQ = dQ_.value();
        params.dQ_d_stride = dQ.dim() == 3 ? dQ.stride(0) : 0;
        params.dQ_r_stride = dQ.dim() == 3 ? dQ.stride(1) : dQ.stride(0);
        params.dQ_dstate_stride = dQ.dim() == 3 ? dQ.stride(2) : dQ.stride(1);
    }
    if (has_z) {
...
selective_scan_fwd(const at::Tensor &u, const at::Tensor &delta,
    ...
    const bool has_pq = P_.has_value() && Q_.has_value();
    TORCH_CHECK(!P_.has_value() || Q_.has_value(), "Q must be provided when P is provided");
    TORCH_CHECK(!Q_.has_value() || P_.has_value(), "P must be provided when Q is provided");
    if (has_pq) {
        TORCH_CHECK(!is_complex, "P/Q dense CUDA forward supports real A only");
        auto P = P_.value();
        auto Q = Q_.value();
        TORCH_CHECK(P.is_cuda() && Q.is_cuda(), "P and Q must be CUDA tensors");
        TORCH_CHECK(P.dim() == Q.dim() && (P.dim() == 2 || P.dim() == 3), "P/Q must be 2D or 3D with same rank");
        if (P.dim() == 2) {
            TORCH_CHECK(P.size(0) == dstate && Q.size(1) == dstate && P.size(1) == Q.size(0),
                        "Expect P(dstate, r), Q(r, dstate)");
        } else {
            TORCH_CHECK(P.size(0) == dim && Q.size(0) == dim, "P/Q dim mismatch");
            TORCH_CHECK(P.size(1) == dstate && Q.size(2) == dstate && P.size(2) == Q.size(1),
                        "Expect P(dim, dstate, r), Q(dim, r, dstate)");
        }
        TORCH_CHECK(P.size(-1) <= 8, "Current dense CUDA forward supports pq_rank <= 8");
    }
    if (gamma_.has_value()) {
        auto gamma = gamma_.value();
        TORCH_CHECK(gamma.is_cuda(), "gamma must be CUDA tensor");
        TORCH_CHECK(gamma.dim() == 1 || gamma.dim() == 2, "gamma must be 1D or 2D");
        if (gamma.dim() == 1) {
            TORCH_CHECK(gamma.size(0) == dstate, "gamma(1D) shape must be (dstate,)");
        } else {
            TORCH_CHECK(gamma.size(0) == dim && gamma.size(1) == dstate, "gamma(2D) shape must be (dim, dstate)");
        }
    }
...
selective_scan_bwd(const at::Tensor &u, const at::Tensor &delta,
                  ...
                  const c10::optional<at::Tensor> &dP_out_,
                  const c10::optional<at::Tensor> &dQ_out_,
                  bool delta_softplus,
                  bool recompute_out_z) {
    auto input_type = u.scalar_type();
    auto weight_type = A.scalar_type();
    ...
    TORCH_CHECK(!Q_.has_value() || P_.has_value(), "P must be provided when Q is provided");
    if (has_pq) {
        TORCH_CHECK(!is_complex, "P/Q dense CUDA backward supports real A only");
        TORCH_CHECK(!z_.has_value(), "P/Q dense CUDA backward v1 does not support z yet");
        auto P = P_.value();
        auto Q = Q_.value();
        TORCH_CHECK(P.is_cuda() && Q.is_cuda(), "P and Q must be CUDA tensors");
        TORCH_CHECK(P.dim() == Q.dim() && (P.dim() == 2 || P.dim() == 3), "P/Q must be 2D or 3D with same rank");
        ...
        TORCH_CHECK(P.size(-1) <= 8, "Current dense CUDA backward supports pq_rank <= 8");
    }
    if (gamma_.has_value()) {
        auto gamma = gamma_.value();
        TORCH_CHECK(gamma.is_cuda(), "gamma must be CUDA tensor");
    ...
    c10::optional<at::Tensor> dP_opt = c10::nullopt, dQ_opt = c10::nullopt;
    if (has_pq) {
        at::Tensor dP = dP_out_.has_value() ? dP_out_.value() : torch::zeros_like(P_.value());
        at::Tensor dQ = dQ_out_.has_value() ? dQ_out_.value() : torch::zeros_like(Q_.value());
        TORCH_CHECK(dP.is_cuda() && dQ.is_cuda(), "dP_out and dQ_out must be CUDA tensors");
        TORCH_CHECK(dP.scalar_type() == at::ScalarType::Float && dQ.scalar_type() == at::ScalarType::Float,
                    "dP_out and dQ_out must be float32");
        TORCH_CHECK(dP.sizes() == P_.value().sizes(), "dP_out shape mismatch");
        TORCH_CHECK(dQ.sizes() == Q_.value().sizes(), "dQ_out shape mismatch");
        dP_opt = dP;
        dQ_opt = dQ;
    }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("fwd", &selective_scan_fwd, "Selective scan forward",
          pybind11::arg("u"), pybind11::arg("delta"), pybind11::arg("A"), pybind11::arg("B"), pybind11::arg("C"),
          pybind11::arg("D") = pybind11::none(), pybind11::arg("z") = pybind11::none(),
          pybind11::arg("delta_bias") = pybind11::none(),
          pybind11::arg("P") = pybind11::none(), pybind11::arg("Q") = pybind11::none(),
          pybind11::arg("gamma") = pybind11::none(),
          pybind11::arg("delta_softplus") = false);
    m.def("bwd", &selective_scan_bwd, "Selective scan backward",
          ...
          pybind11::arg("P") = pybind11::none(), pybind11::arg("Q") = pybind11::none(),
          pybind11::arg("gamma") = pybind11::none(),
          pybind11::arg("dout"), pybind11::arg("x"), pybind11::arg("out"), pybind11::arg("dz"),
          pybind11::arg("dP_out") = pybind11::none(), pybind11::arg("dQ_out") = pybind11::none(),
          pybind11::arg("delta_softplus"), pybind11::arg("recompute_out_z"));
}
