...
# Evaluate base mamba130m vs motifmamba130m on HellaSwag (hellaS).
...
OUT_ROOT=${OUT_ROOT:-${ROOT}/motif/logs/hellas_compare}
STAMP=$(date +%Y%m%d_%H%M%S)
RUN_DIR="${OUT_ROOT}/mamba130m_vs_motif130m_hellas_${STAMP}"
...
"${PY}" -m lm_eval run \
  ...
  --cache_requests true \
  --use_cache "${ROOT}/.cache/lm_eval/hellas_base_mamba130m.sqlite" \
  "${EXTRA_ARGS[@]}" \
...
"${PY}" "${PQ_WRAPPER}" run \
  ...
  --cache_requests true \
  --use_cache "${ROOT}/.cache/lm_eval/hellas_motifmamba130m.sqlite" \
  "${EXTRA_ARGS[@]}" \
