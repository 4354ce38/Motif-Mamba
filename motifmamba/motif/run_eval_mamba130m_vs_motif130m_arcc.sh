...
# Evaluate base mamba130m vs motifmamba130m on ARC-Challenge (arcc).
...
TASKS=${TASKS:-arc_challenge}
OUT_ROOT=${OUT_ROOT:-${ROOT}/motif/logs/arcc_compare}
STAMP=$(date +%Y%m%d_%H%M%S)
RUN_DIR="${OUT_ROOT}/mamba130m_vs_motif130m_arcc_${STAMP}"
...
"${PY}" -m lm_eval run \
  ...
  --cache_requests true \
  --use_cache "${ROOT}/.cache/lm_eval/arcc_base_mamba130m.sqlite" \
  "${EXTRA_ARGS[@]}" \
...
"${PY}" "${PQ_WRAPPER}" run \
  ...
  --cache_requests true \
  --use_cache "${ROOT}/.cache/lm_eval/arcc_motifmamba130m.sqlite" \
  "${EXTRA_ARGS[@]}" \
...
with open(detail_txt, "w", encoding="utf-8") as f:
    for k, r in enumerate(rows, 1):
        ...
        f.write(f"Mamba: {r['base_pred_idx']} -> {r['base_pred_choice']} (scores: {r['base_scores']})\n")
        f.write(f"MotifMamba: {r['motif_pred_idx']} -> {r['motif_pred_choice']} (scores: {r['motif_scores']})\n\n")
