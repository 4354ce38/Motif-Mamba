...
# Evaluate base mamba130m vs motifmamba130m on PIQA (piqa).
...
TASKS=${TASKS:-piqa}
OUT_ROOT=${OUT_ROOT:-${ROOT}/motif/logs/piqa_compare}
STAMP=$(date +%Y%m%d_%H%M%S)
RUN_DIR="${OUT_ROOT}/mamba130m_vs_motif130m_piqa_${STAMP}"
...
"${PY}" -m lm_eval run \
  ...
  --cache_requests true \
  --use_cache "${ROOT}/.cache/lm_eval/piqa_base_mamba130m.sqlite" \
  "${EXTRA_ARGS[@]}" \
...
"${PY}" "${PQ_WRAPPER}" run \
  ...
  --cache_requests true \
  --use_cache "${ROOT}/.cache/lm_eval/piqa_motifmamba130m.sqlite" \
  "${EXTRA_ARGS[@]}" \
