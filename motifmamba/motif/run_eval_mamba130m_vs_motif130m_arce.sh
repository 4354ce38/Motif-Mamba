...
# Evaluate base mamba130m vs motifmamba130m on ARC-Easy (arce).
...
OUT_ROOT=${OUT_ROOT:-${ROOT}/motif/logs/arce_compare}
STAMP=$(date +%Y%m%d_%H%M%S)
RUN_DIR="${OUT_ROOT}/mamba130m_vs_motif130m_arce_${STAMP}"
...
"${PY}" -m lm_eval run \
  ...
  --cache_requests true \
  --use_cache "${ROOT}/.cache/lm_eval/arce_base_mamba130m.sqlite" \
  "${EXTRA_ARGS[@]}" \
...
"${PY}" "${PQ_WRAPPER}" run \
  ...
  --cache_requests true \
  --use_cache "${ROOT}/.cache/lm_eval/arce_motifmamba130m.sqlite" \
  "${EXTRA_ARGS[@]}" \
