...
class SelectiveScanPQFn(torch.autograd.Function):
    ...
    def backward(ctx, dout, *args):
        ...
        pq_grad_mode = os.environ.get("MOTIFMAMBA_PQ_GRAD_MODE", "exact").strip().lower()
        use_exact_ref_grad = pq_grad_mode in ("exact", "ref", "reference", "full")
        dP_kernel = None
        dQ_kernel = None
        if not use_exact_ref_grad:
            # Fast fused mode: let CUDA kernel write dP/dQ directly.
            dP_kernel = torch.zeros_like(P, dtype=torch.float32, device=P.device)
            dQ_kernel = torch.zeros_like(Q, dtype=torch.float32, device=Q.device)
        ...
        du, ddelta, dA, dB, dC, dD, ddelta_bias, *rest = selective_scan_cuda.bwd(
            ...
            dP_out=dP_kernel, dQ_out=dQ_kernel,
            delta_softplus=ctx.delta_softplus,
            recompute_out_z=False
        )

        if use_exact_ref_grad:
